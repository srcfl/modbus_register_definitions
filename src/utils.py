...
def decode_jwt_to_json(jwt_token: str) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    try:
        ...
        header = json.loads(base64.urlsafe_b64decode(
            header_pad).decode('utf-8'))

        # Decode payload
        decoded_payload = jwt.decode(jwt_token, algorithms=['ES256'], options={
                                     "verify_signature": False}, key=None)
...
def create_register_values_from_jwt(header: Dict[str, Any], payload: Dict[str, Any]) -> List[Tuple[RegisterDefinition, Any]]:
    ...
    if model not in INVERTER_PROFILES:
        raise ValueError(
            f"Unsupported model type: {model}. Supported models: {', '.join(INVERTER_PROFILES.keys())}")
        ...
        if str(start_reg) in register_data:
            ...
            for i in range(num_regs):
                ...
                if reg_addr in register_data:
                    ...
                    raw_bytes.extend(reg_value.to_bytes(
                        2, 'big', signed=False))
